    ...
    inout standard_metadata_t   std_meta)
{
    ...
    action read_classified_flag(bit<INDEX_WIDTH> register_index) {
        reg_classified_flag.read(meta.classified_flag, register_index);
    }

    /* Register update action */
    action update_classified_flag(bit<INDEX_WIDTH> register_index) {
        if (meta.pkt_count == 3) {
            reg_classified_flag.write(register_index, meta.final_class);
        }
    }
    ...
    action update_flow_ID(bit<INDEX_WIDTH> register_index) {
        reg_flow_ID.write(register_index, meta.flow_ID);
    }
    /* Register read action */
    bit<32> tmp_flow_ID;
    action read_only_flow_ID(bit<INDEX_WIDTH> register_index) {
        reg_flow_ID.read(tmp_flow_ID, register_index);
    }

    register<bit<32>>(MAX_REGISTER_ENTRIES) reg_time_last_pkt;
    /* Register read action */
    action read_time_last_pkt(bit<INDEX_WIDTH> register_index) {
        reg_time_last_pkt.read(meta.time_last_pkt, register_index);
        reg_time_last_pkt.write(register_index, std_meta.ingress_global_timestamp[31:0]);
    }
    ...
    action read_pkt_count(bit<INDEX_WIDTH> register_index) {
        reg_pkt_count.read(meta.pkt_count, register_index);
        meta.pkt_count = meta.pkt_count + 1;
        reg_pkt_count.write(register_index, meta.pkt_count);
    }
	...
	table code_table2{
        key = {
            ...
            meta.codeword2_3: ternary;
            meta.codeword2_4: ternary;
            meta.codeword2_5: ternary;
            }
    ...
    apply {
        ...
        if (meta.f_action != 0) {
            ...
            if (meta.pkt_count < 4){
                ...
                digest<flow_class_digest>(1,{hdr.ipv4.src_addr, hdr.ipv4.dst_addr, meta.hdr_srcport, meta.hdr_dstport, hdr.ipv4.protocol, meta.final_class, meta.pkt_count, meta.register_index});
